...
def build_3DUnet(input_shape, use_upsampling=False, initial_learning_rate=0.5, stride=1, kernel_size=3):
    ...
    # Layers with maxpool
    conv1 = Conv3D(filters=n_base_filters, kernel_size=kernel_size, strides=stride, activation=activation, padding=padding)(inputs)
    conv2 = Conv3D(filters=n_base_filters*2, kernel_size=kernel_size, strides=stride, activation=activation, padding=padding)(conv1)
    maxpool1 = MaxPooling3D(pool_size=2,)(conv2)

    conv3 = Conv3D(filters=n_base_filters*2, kernel_size=kernel_size, strides=stride, activation=activation, padding=padding)(maxpool1)
    conv4 = Conv3D(filters=n_base_filters*4, kernel_size=kernel_size, strides=stride, activation=activation, padding=padding)(conv3)
    maxpool2 = MaxPooling3D(pool_size=2)(conv4)

    conv5 = Conv3D(filters=n_base_filters*4, kernel_size=kernel_size, strides=stride, activation=activation, padding=padding)(maxpool2)
    conv6 = Conv3D(filters=n_base_filters*8, kernel_size=kernel_size, strides=stride, activation=activation, padding=padding)(conv5)
    ...
    conv7 = Conv3D(filters=n_base_filters*8, kernel_size=kernel_size, strides=stride, activation=activation, padding=padding)(maxpool3)
    conv8 = Conv3D(filters=n_base_filters*16, kernel_size=kernel_size, strides=stride, activation=activation, padding=padding)(conv7)
    upsampling1 = get_upconvolution(n_base_filters*16, use_upsampling)(conv8)
    concat1 = keras.layers.concatenate([upsampling1 ,conv6])

    conv9 = Conv3D(filters=n_base_filters*8, kernel_size=kernel_size, strides=stride, activation=activation, padding=padding)(concat1)
    conv10 = Conv3D(filters=n_base_filters*8, kernel_size=kernel_size, strides=stride, activation=activation, padding=padding)(conv9)
    upsampling2 = get_upconvolution(n_base_filters*8, use_upsampling)(conv10)
    concat2 = keras.layers.concatenate([upsampling2 ,conv4])

    conv11 = Conv3D(filters=n_base_filters*4, kernel_size=kernel_size, strides=stride, activation=activation, padding=padding)(concat2)
    conv12 = Conv3D(filters=n_base_filters*4, kernel_size=kernel_size, strides=stride, activation=activation, padding=padding)(conv11)
    upsampling3 = get_upconvolution(n_base_filters*4, use_upsampling)(conv12)
    concat3 = keras.layers.concatenate([upsampling3 ,conv2])

    # Final layers
    conv13 = Conv3D(filters=n_base_filters*2, kernel_size=kernel_size, strides=stride, activation=activation, padding=padding)(concat3)
