...
class Trainer3DUnet:
    ...
    def train(self, data_file_location, label_file_location, n_epochs, save_name, batch_size=1, use_cross_validation=False, validation_label_location="", validation_data_location=""):
        # Loads the files
    ...
    def get_generator(self, data, labels, mini_batch_size=4, using_sparse_categorical_crossentropy=False):
        while True:
            ...
            x_list = np.zeros((mini_batch_size, 176, 208, 176, 1))
            y_list = np.zeros((mini_batch_size, 176, 208, 176, 1))
